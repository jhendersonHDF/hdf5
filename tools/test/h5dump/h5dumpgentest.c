...
gent_complex(void)
{
    ...
    mem_tid = H5Tcreate(H5T_COMPOUND, sizeof(float _Complex));
...
gent_complex_be(void)
{
    ...
    struct {
        float _Complex arr[F96_XDIM][F96_YDIM];
    } * dset_fc;

    struct {
        double _Complex arr[F96_XDIM][F96_YDIM];
    } * dset_dc;

    struct {
        long double _Complex arr[F96_XDIM][F96_YDIM];
    } * dset_ldc;
    ...
    for (size_t i = 0; i < dims[0]; i++) {
        ...
        for (size_t j = 1; j < dims[1]; j++) {
            float _Complex z = (float)(j * dims[0] + i) / (float)F96_YDIM;
    ...
    for (size_t i = 0; i < dims[0]; i++) {
        ...
        for (size_t j = 1; j < dims[1]; j++) {
            double _Complex z = (double)(j * dims[0] + i) / (double)F96_YDIM;
    ...
    val = (long double)F96_YDIM;
    for (size_t i = 0; i < dims[0]; i++) {
        dset_ldc->arr[i][0] = (long double _Complex)val;

        for (size_t j = 1; j < dims[1]; j++) {
            long double _Complex z = (long double)(j * dims[0] + i) / (long double)F96_YDIM;
