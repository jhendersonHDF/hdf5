...
test_particular_fp_integer(void)
{
    ...
    float _Complex src_fc     = (float _Complex)INT_MAX;
    size_t src_size4;
    size_t dst_size4;
#endif
#endif
...
/* Only run this part of the test if conversion exceptions are enabled */
...
#ifdef H5_HAVE_COMPLEX_NUMBERS
    /* Test conversion from float _Complex (the value is INT_MAX) to int. */
    ...
    /* Print errors */
    if (dst_i != fill_value) {
        float _Complex x = 0.0F + 0.0F * _Complex_I;
        ...
        printf("    test float _Complex to int:\n");
        printf("        src = ");
        buf_ptr = saved_buf4;
...
my_isnan(dtype_t type, void *val)
{
    ...
    if (FLT_FLOAT == type) {
        ...
        float _Complex x;
        float x_real, x_imag;
        memcpy(&x, val, sizeof(float _Complex));
        x_real = crealf(x);
        x_imag = cimagf(x);
        ...
        memcpy(&x, val, sizeof(double _Complex));
        x_real = creal(x);
        x_imag = cimag(x);
        if (isinf(x_real) || isinf(x_imag))
            retval = 0;
        ...
        memcpy(&x, val, sizeof(long double _Complex));
        x_real = creall(x);
        x_imag = cimagl(x);
        if (isinf(x_real) || isinf(x_imag))
            retval = 0;
...
#ifdef H5_HAVE_COMPLEX_NUMBERS
        case FLT_COMPLEX:
            *((float _Complex *)hw_dst) = (float _Complex)aligned;
            break;
        case DBL_COMPLEX:
            *((double _Complex *)hw_dst) = (double _Complex)aligned;
            break;
        case LDBL_COMPLEX:
            *((long double _Complex *)hw_dst) = (long double _Complex)aligned;
            break;
#else
        case FLT_COMPLEX:
        case DBL_COMPLEX:
 ...
 *
 * Purpose:     Helper function for test_conv_flt_1 to perform conversion
 *              from float _Complex to another type by casting. Also checks
 *              for overflow and underflow when the destination type is a
 *              type with a smaller width than float.
 *
 * Return:      enum conv_func_ret_t value
 ...
 *
 * Purpose:     Helper function for test_conv_flt_1 to perform conversion
 *              from double _Complex to another type by casting. Also
 *              checks for overflow and underflow when the destination type
 *              is a type with a smaller width than double.
 *
 * Return:      enum conv_func_ret_t value
 *
 *-------------------------------------------------------------------------
...
{
    double _Complex aligned;
    double          real_val, imag_val;
    conv_func_ret_t ret = CONV_SUCCESS;

    memcpy(&aligned, src_buf + idx * sizeof(double _Complex), sizeof(double _Complex));
    ...
    switch (dst_type) {
        ...
        case DBL_COMPLEX:
            *((double _Complex *)hw_dst) = aligned;
            break;
        case LDBL_COMPLEX:
            *((long double _Complex *)hw_dst) = (long double _Complex)aligned;
            break;
 ...
 *
 * Purpose:     Helper function for test_conv_flt_1 to perform conversion
 *              from long double _Complex to another type by casting. Also
 *              checks for overflow and underflow when the destination type
 *              is a type with a smaller width than long double.
 *
 * Return:      enum conv_func_ret_t value
 *
 *-------------------------------------------------------------------------
...
test_conv_flt_1_hw_conv_from_lcomplex(void *hw_dst, unsigned char *src_buf, size_t idx, dtype_t dst_type)
{
    ...
    switch (dst_type) {
        ...
        case LDBL_COMPLEX:
            *((long double _Complex *)hw_dst) = aligned;
            break;
#else
        case FLT_COMPLEX:
        case DBL_COMPLEX:
...
test_conv_flt_1(const char *name, int run_test, hid_t src, hid_t dst)
{
    ...
    double _Complex hw_double_complex;
    float _Complex hw_float_complex;
#endif
    ...
    for (j = 0; j < nelmts; j++) {
...
#endif
#ifdef H5_HAVE_COMPLEX_NUMBERS
        hw_float_complex   = 911.0F + 911.0F * (float _Complex)_Complex_I;
        hw_double_complex  = 911.0 + 911.0 * (double _Complex)_Complex_I;
        hw_ldouble_complex = 911.0L + 911.0L * (long double _Complex)_Complex_I;
#endif
...
#endif
#ifdef H5_HAVE_COMPLEX_NUMBERS
        else if (FLT_COMPLEX == dst_type && my_isnan(dst_type, buf + j * sizeof(float _Complex)) &&
                 my_isnan(dst_type, hw)) {
            continue;
        }
        else if (DBL_COMPLEX == dst_type && my_isnan(dst_type, buf + j * sizeof(double _Complex)) &&
                 my_isnan(dst_type, hw)) {
            continue;
        }
        else if (LDBL_COMPLEX == dst_type && my_isnan(dst_type, buf + j * sizeof(long double _Complex)) &&
                 my_isnan(dst_type, hw)) {
            continue;
        }
#endif
        ...
        if (FLT_FLOAT == src_type) {
            ...
            float _Complex fc;
            memcpy(&fc, &saved[j * src_size], sizeof(float _Complex));
            printf(" %29.20e%+.20ei\n", (double)crealf(fc), (double)cimagf(fc));
        }
        else if (DBL_COMPLEX == src_type) {
            double _Complex dc;
            memcpy(&dc, &saved[j * src_size], sizeof(double _Complex));
            printf(" %29.20e%+.20ei\n", creal(dc), cimag(dc));
        }
        else if (LDBL_COMPLEX == src_type) {
            long double _Complex ldc;
            memcpy(&ldc, &saved[j * src_size], sizeof(long double _Complex));
...
test_conv_int_fp_conv_to_schar(void *hw_p, unsigned char *src_buf, size_t idx, dtype_t src_type)
{
    ...
    switch (src_type) {
        ...
        case FLT_COMPLEX: {
            float _Complex fc;
            memcpy(&fc, src_buf + idx * sizeof(float _Complex), sizeof(float _Complex));
            aligned = (signed char)fc;
            break;
        }
        case DBL_COMPLEX: {
            double _Complex dc;
            memcpy(&dc, src_buf + idx * sizeof(double _Complex), sizeof(double _Complex));
            aligned = (signed char)dc;
            break;
        }
        case LDBL_COMPLEX: {
            long double _Complex ldc;
            memcpy(&ldc, src_buf + idx * sizeof(long double _Complex), sizeof(long double _Complex));
 ...
 *
 * Purpose:     Helper function for test_conv_int_fp to perform conversion
 *              from a datatype to float _Complex by casting.
 *
 * Return:      -1 on failure
 *              0 on success
 *
...
test_conv_int_fp_conv_to_fcomplex(void *hw_p, unsigned char *src_buf, size_t idx, dtype_t src_type)
{
    ...
    memset(&aligned, 0, sizeof(float _Complex));

    switch (src_type) {
        case INT_SCHAR: {
            signed char c;
            memcpy(&c, src_buf + idx * sizeof(signed char), sizeof(signed char));
            aligned = (float _Complex)c;
            break;
        }
        case INT_UCHAR: {
            unsigned char uc;
            memcpy(&uc, src_buf + idx * sizeof(unsigned char), sizeof(unsigned char));
            aligned = (float _Complex)uc;
            break;
        }
        case INT_SHORT: {
            short s;
            memcpy(&s, src_buf + idx * sizeof(short), sizeof(short));
            aligned = (float _Complex)s;
            break;
        }
        case INT_USHORT: {
            unsigned short us;
            memcpy(&us, src_buf + idx * sizeof(unsigned short), sizeof(unsigned short));
            aligned = (float _Complex)us;
            break;
        }
        case INT_INT: {
            int i;
            memcpy(&i, src_buf + idx * sizeof(int), sizeof(int));
            aligned = (float _Complex)i;
            break;
        }
        case INT_UINT: {
            unsigned int ui;
            memcpy(&ui, src_buf + idx * sizeof(unsigned int), sizeof(unsigned int));
            aligned = (float _Complex)ui;
            break;
        }
        case INT_LONG: {
            long l;
            memcpy(&l, src_buf + idx * sizeof(long), sizeof(long));
            aligned = (float _Complex)l;
            break;
        }
        case INT_ULONG: {
            unsigned long ul;
            memcpy(&ul, src_buf + idx * sizeof(unsigned long), sizeof(unsigned long));
            aligned = (float _Complex)ul;
            break;
        }
        case INT_LLONG: {
            long long ll;
            memcpy(&ll, src_buf + idx * sizeof(long long), sizeof(long long));
            aligned = (float _Complex)ll;
            break;
        }
        case INT_ULLONG: {
            unsigned long long ull;
            memcpy(&ull, src_buf + idx * sizeof(unsigned long long), sizeof(unsigned long long));
            aligned = (float _Complex)ull;
            break;
        }
        case FLT_FLOAT16:
        case FLT_FLOAT:
 ...
 *
 * Purpose:     Helper function for test_conv_int_fp to perform conversion
 *              from a datatype to double _Complex by casting.
 *
 * Return:      -1 on failure
 *              0 on success
 *
...
test_conv_int_fp_conv_to_dcomplex(void *hw_p, unsigned char *src_buf, size_t idx, dtype_t src_type)
{
    ...
    memset(&aligned, 0, sizeof(double _Complex));

    switch (src_type) {
        case INT_SCHAR: {
            signed char c;
            memcpy(&c, src_buf + idx * sizeof(signed char), sizeof(signed char));
            aligned = (double _Complex)c;
            break;
        }
        case INT_UCHAR: {
            unsigned char uc;
            memcpy(&uc, src_buf + idx * sizeof(unsigned char), sizeof(unsigned char));
            aligned = (double _Complex)uc;
            break;
        }
        case INT_SHORT: {
            short s;
            memcpy(&s, src_buf + idx * sizeof(short), sizeof(short));
            aligned = (double _Complex)s;
            break;
        }
        case INT_USHORT: {
            unsigned short us;
            memcpy(&us, src_buf + idx * sizeof(unsigned short), sizeof(unsigned short));
            aligned = (double _Complex)us;
            break;
        }
        case INT_INT: {
            int i;
            memcpy(&i, src_buf + idx * sizeof(int), sizeof(int));
            aligned = (double _Complex)i;
            break;
        }
        case INT_UINT: {
            unsigned int ui;
            memcpy(&ui, src_buf + idx * sizeof(unsigned int), sizeof(unsigned int));
            aligned = (double _Complex)ui;
            break;
        }
        case INT_LONG: {
            long l;
            memcpy(&l, src_buf + idx * sizeof(long), sizeof(long));
            aligned = (double _Complex)l;
            break;
        }
        case INT_ULONG: {
            unsigned long ul;
            memcpy(&ul, src_buf + idx * sizeof(unsigned long), sizeof(unsigned long));
            aligned = (double _Complex)ul;
            break;
        }
        case INT_LLONG: {
            long long ll;
            memcpy(&ll, src_buf + idx * sizeof(long long), sizeof(long long));
            aligned = (double _Complex)ll;
            break;
        }
        case INT_ULLONG: {
            unsigned long long ull;
            memcpy(&ull, src_buf + idx * sizeof(unsigned long long), sizeof(unsigned long long));
            aligned = (double _Complex)ull;
            break;
        }
        case FLT_FLOAT16:
        case FLT_FLOAT:
 ...
 *
 * Purpose:     Helper function for test_conv_int_fp to perform conversion
 *              from a datatype to long double _Complex by casting.
 *
 * Return:      -1 on failure
 *              0 on success
 *
...
test_conv_int_fp_conv_to_lcomplex(void *hw_p, unsigned char *src_buf, size_t idx, dtype_t src_type)
{
    ...
    memset(&aligned, 0, sizeof(long double _Complex));

    switch (src_type) {
        case INT_SCHAR: {
            signed char c;
            memcpy(&c, src_buf + idx * sizeof(signed char), sizeof(signed char));
            aligned = (long double _Complex)c;
            break;
        }
        case INT_UCHAR: {
            unsigned char uc;
            memcpy(&uc, src_buf + idx * sizeof(unsigned char), sizeof(unsigned char));
            aligned = (long double _Complex)uc;
            break;
        }
        case INT_SHORT: {
            short s;
            memcpy(&s, src_buf + idx * sizeof(short), sizeof(short));
            aligned = (long double _Complex)s;
            break;
        }
        case INT_USHORT: {
            unsigned short us;
            memcpy(&us, src_buf + idx * sizeof(unsigned short), sizeof(unsigned short));
            aligned = (long double _Complex)us;
            break;
        }
        case INT_INT: {
            int i;
            memcpy(&i, src_buf + idx * sizeof(int), sizeof(int));
            aligned = (long double _Complex)i;
            break;
        }
        case INT_UINT: {
            unsigned int ui;
            memcpy(&ui, src_buf + idx * sizeof(unsigned int), sizeof(unsigned int));
            aligned = (long double _Complex)ui;
            break;
        }
        case INT_LONG: {
            long l;
            memcpy(&l, src_buf + idx * sizeof(long), sizeof(long));
            aligned = (long double _Complex)l;
            break;
        }
        case INT_ULONG: {
            unsigned long ul;
            memcpy(&ul, src_buf + idx * sizeof(unsigned long), sizeof(unsigned long));
            aligned = (long double _Complex)ul;
            break;
        }
        case INT_LLONG: {
            long long ll;
            memcpy(&ll, src_buf + idx * sizeof(long long), sizeof(long long));
            aligned = (long double _Complex)ll;
            break;
        }
        case INT_ULLONG: {
            unsigned long long ull;
            memcpy(&ull, src_buf + idx * sizeof(unsigned long long), sizeof(unsigned long long));
            aligned = (long double _Complex)ull;
            break;
        }
        case FLT_FLOAT16:
        case FLT_FLOAT:
    ...
    *((long double _Complex *)hw_p) = aligned;
...
test_conv_int_fp(const char *name, int run_test, hid_t src, hid_t dst)
{
...
#ifdef H5_HAVE_COMPLEX_NUMBERS
    memset(&hw_ldouble_complex, 0, sizeof(long double _Complex));
#endif
    ...
    for (j = 0; j < nelmts; j++) {
        ...
        switch (src_type) {
            ...
            case FLT_COMPLEX: {
                float _Complex fc;
                memcpy(&fc, saved + j * sizeof(float _Complex), sizeof(float _Complex));
                printf(" %29f%+fi\n", (double)crealf(fc), (double)cimagf(fc));
                break;
            }
            case DBL_COMPLEX: {
                double _Complex dc;
                memcpy(&dc, saved + j * sizeof(double _Complex), sizeof(double _Complex));
                printf(" %29f%+fi\n", creal(dc), cimag(dc));
                break;
            }
            case LDBL_COMPLEX: {
                long double _Complex ldc;
                memcpy(&ldc, saved + j * sizeof(long double _Complex), sizeof(long double _Complex));
                printf(" %29Lf%+Lfi\n", creall(ldc), cimagl(ldc));
                break;
            }
#else
        ...
        switch (dst_type) {
            ...
            case FLT_COMPLEX: {
                float _Complex fc;
                memcpy(&fc, buf + j * sizeof(float _Complex), sizeof(float _Complex));
                printf(" %29f%+fi\n", (double)crealf(fc), (double)cimagf(fc));
                break;
            }
            case DBL_COMPLEX: {
                double _Complex dc;
                memcpy(&dc, buf + j * sizeof(double _Complex), sizeof(double _Complex));
                printf(" %29f%+fi\n", creal(dc), cimag(dc));
                break;
            }
            case LDBL_COMPLEX: {
                long double _Complex ldc;
                memcpy(&ldc, buf + j * sizeof(long double _Complex), sizeof(long double _Complex));
                printf(" %29Lf%+Lfi\n", creall(ldc), cimagl(ldc));
                break;
            }
#else
...
#ifdef H5_HAVE_COMPLEX_NUMBERS
            case FLT_COMPLEX:
                printf(" %29f%+fi\n", (double)crealf(*((float _Complex *)((void *)hw))),
                       (double)cimagf(*((float _Complex *)((void *)hw))));
                break;
            case DBL_COMPLEX:
                printf(" %29f%+fi\n", creal(*((double _Complex *)((void *)hw))),
                       cimag(*((double _Complex *)((void *)hw))));
                break;
            case LDBL_COMPLEX:
                printf(" %29Lf%+Lfi\n", creall(*((long double _Complex *)((void *)hw))),
                       cimagl(*((long double _Complex *)((void *)hw))));
                break;
#else
            case FLT_COMPLEX:
            case DBL_COMPLEX:
