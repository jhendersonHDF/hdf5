...
{
    float _Complex wdata[SPACE1_DIM1][ARRAY1_DIM1]; /* Information to write         */
    float _Complex rdata[SPACE1_DIM1][ARRAY1_DIM1]; /* Information read in          */
    H5T_class_t mclass;                             /* Datatype class for VL        */
    ...
    for (size_t i = 0; i < SPACE1_DIM1; i++)
        for (size_t j = 0; j < ARRAY1_DIM1; j++)
            wdata[i][j] = (float _Complex)((float)(i * 100) + ((float)(j * 10) * _Complex_I));
    ...
    for (size_t i = 0; i < SPACE1_DIM1; i++) {
        for (size_t j = 0; j < ARRAY1_DIM1; j++) {
            if (0 != memcmp(&wdata[i][j], &rdata[i][j], sizeof(float _Complex)))
                TestErrPrintf(
